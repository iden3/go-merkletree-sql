...
func TestTreeRootWithOneNode(t *testing.T, sto merkletree.Storage) {
	...
	_, err = mt.Add(ctx, big.NewInt(100), big.NewInt(200))
	assert.Nil(t, err)
	assert.Equal(t,
...
func TestAddDifferentOrder(t *testing.T, sto merkletree.Storage,
	...
	for i := 0; i < 16; i++ {
		...
		v := big.NewInt(0)
		if _, err := mt1.Add(ctx, k, v); err != nil {
			t.Fatal(err)
		}
	}
	...
	for i := 16 - 1; i >= 0; i-- {
		k := big.NewInt(int64(i))
		v := big.NewInt(0)
		if _, err := mt2.Add(ctx, k, v); err != nil {
			t.Fatal(err)
		}
	}
...
func TestAddRepeatedIndex(t *testing.T, sto merkletree.Storage) {
	...
	ctx := context.Background()
	if _, err := mt.Add(ctx, k, v); err != nil {
		t.Fatal(err)
	}
...
func TestUpdate2(t *testing.T, sto merkletree.Storage) {
	...
	_, err := mt1.Add(ctx, big.NewInt(1), big.NewInt(119))
	assert.Nil(t, err)
	_, err = mt1.Add(ctx, big.NewInt(2), big.NewInt(229))
	assert.Nil(t, err)
	_, err = mt1.Add(ctx, big.NewInt(9876), big.NewInt(6789))
	assert.Nil(t, err)

	_, err = mt2.Add(ctx, big.NewInt(1), big.NewInt(11))
	assert.Nil(t, err)
	_, err = mt2.Add(ctx, big.NewInt(2), big.NewInt(22))
	assert.Nil(t, err)
	_, err = mt2.Add(ctx, big.NewInt(9876), big.NewInt(10))
	assert.Nil(t, err)
...
func TestVerifyProofCases(t *testing.T, sto merkletree.Storage) {
	...
	ctx := context.Background()
	for i := 0; i < 8; i++ {
		if _, err := mt.Add(ctx, big.NewInt(int64(i)), big.NewInt(0)); err != nil {
			t.Fatal(err)
		}
	}
	...
	assert.Equal(t,
		"0003529cbedbda2bdd25fd6455551e55245fa6dc11a9d0c27dc0cd38fca44c17e40344ad686a18ba78b502c0b6f285c5c8393bde2f7a3e2abe586515e4d84533e3037b062539bde2d80749746986cf8f0001fd2cdbf9a89fcbf981a769daef49df06", //nolint:lll
	...
	assert.Equal(t,
		"", //nolint:lll
		hex.EncodeToString(proof.Bytes()))
	...
	assert.Equal(t,
		"", //nolint:lll
		hex.EncodeToString(proof.Bytes()))
}
...
func TestDelete(t *testing.T, sto merkletree.Storage) {
	...
	_, err = mt.Delete(ctx, big.NewInt(1234))
	assert.Nil(t, err)
	_, err = mt.Delete(ctx, big.NewInt(1))
	assert.Nil(t, err)
	assert.Equal(t, "0", mt.Root().String())
...
func TestDelete2(t *testing.T, sto merkletree.Storage,
	...
	_, err = mt.Delete(ctx, big.NewInt(8))
	assert.Nil(t, err)
...
func TestDelete3(t *testing.T, sto merkletree.Storage,
	...
	ctx := context.Background()
	_, err := mt.Add(ctx, big.NewInt(1), big.NewInt(1))
	assert.Nil(t, err)

	_, err = mt.Add(ctx, big.NewInt(2), big.NewInt(2))
	assert.Nil(t, err)
	...
	mt2 := newTestingMerkle(t, sto2, 140)
	_, err = mt2.Add(ctx, big.NewInt(2), big.NewInt(2))
	assert.Nil(t, err)
	assert.Equal(t, mt2.Root(), mt.Root())
}
...
func TestDelete4(t *testing.T, sto merkletree.Storage,
	...
	_, err = mt.Add(ctx, big.NewInt(3), big.NewInt(3))
	assert.Nil(t, err)
	...
	_, err = mt2.Add(ctx, big.NewInt(2), big.NewInt(2))
	assert.Nil(t, err)
	_, err = mt2.Add(ctx, big.NewInt(3), big.NewInt(3))
	assert.Nil(t, err)
	assert.Equal(t, mt2.Root(), mt.Root())
}
...
func TestDelete5(t *testing.T, sto merkletree.Storage,
	...
	mt2 := newTestingMerkle(t, sto2, 140)
	_, err = mt2.Add(ctx, big.NewInt(33), big.NewInt(44))
	assert.Nil(t, err)
	assert.Equal(t, mt2.Root(), mt.Root())
}
...
func TestAddAndGetCircomProof(t *testing.T, sto merkletree.Storage) {
	...
	// test vectors generated using https://github.com/iden3/circomlib smt.js
	cpp, err := mt.Add(ctx, big.NewInt(1), big.NewInt(2))
	assert.Nil(t, err)
...
func TestTypesMarshalers(t *testing.T, sto merkletree.Storage) {
	...
	// test TransactionInfo marshalers
	b, err := json.Marshal(&cpp)
	assert.Nil(t, err)

	var cpp2 *merkletree.TransactionInfo
